...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, convs_kernel_sizes, convs_padding_sizes, dropout=0.1):
        super().__init__()

        # Use Conv1D
        # position-wise
        self.w_1 = nn.Conv1d(
            d_in, d_hid, kernel_size=convs_kernel_sizes[0], padding=convs_padding_sizes[0])
        # position-wise
        self.w_2 = nn.Conv1d(
            d_hid, d_in, kernel_size=convs_kernel_sizes[1], padding=convs_padding_sizes[1])
...
class FFTBlock(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 d_k,
                 d_v,
                 kernel_sizes, padding_sizes,
                 dropout=0.1):
        super(FFTBlock, self).__init__()
        self.slf_attn = MultiHeadAttention(
            n_head, d_model, d_k, d_v, dropout=dropout)
        self.pos_ffn = PositionwiseFeedForward(
            d_model, d_inner, kernel_sizes, padding_sizes, dropout=dropout)
